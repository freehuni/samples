...
#ifdef IGD_WebClient_SESSION_TRACKING
void IGD_WebClient_SessionTrack(void *RequestToken, void *Session, char *msg)
{
#if defined(WIN32) || defined(_WIN32_WCE)
	char tempMsg[4096];
...
#define SESSION_TRACK(RequestToken,Session,msg) IGD_WebClient_SessionTrack(RequestToken,Session,msg)
#else
...
struct IGD_WebClient_StreamedRequestBuffer
{
	char *buffer;
...
struct IGD_WebClient_StreamedRequestState
{
...
struct IGD_WebClientManager
{
	...
	SSL_CTX *ssl_ctx;
	IGD_WebClient_OnSslConnection OnSslConnection;
	#endif

	//typedef void(*IGD_WebClient_OnSslConnection)(IGD_WebClient_StateObject sender, X509 *x509, void *user1, void *user2);
	//typedef void(*IGD_AsyncSocket_OnSslConnection)(IGD_AsyncSocket_SocketModule AsyncSocketToken, X509 *x509, void *user);
};
//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
struct IGD_WebClient_ChunkData
{
	int Flag;
	char *buffer;
	int offset;
...
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
struct IGD_WebClientDataObject
{
	int PipelineFlag;
	int ActivityCounter;
	struct sockaddr_in6 remote;
	struct IGD_WebClientManager *Parent;
	...
	int Chunked;
//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
	struct IGD_WebClient_ChunkData *chunk;
...
struct IGD_WebClient_PipelineRequestToken
{
	struct IGD_WebClientDataObject *wcdo;
	void* timer;
};

struct IGD_WebRequest
{
	char **Buffer;
	int *BufferLength;
	int *UserFree;
	...
	struct IGD_WebClient_PipelineRequestToken *requestToken;
	struct IGD_WebClient_StreamedRequestState *streamedState;
...
void IGD_WebClient_DestroyWebRequest(struct IGD_WebRequest *wr)
{
	int i;
	struct IGD_WebClient_StreamedRequestBuffer *b;

	if (wr == NULL) return;

	if (wr->streamedState != NULL)
	{
		while (IGD_Queue_IsEmpty(wr->streamedState->BufferQueue) == 0)
		{
			b = (struct IGD_WebClient_StreamedRequestBuffer*)IGD_Queue_DeQueue(wr->streamedState->BufferQueue);
			if (b == NULL) break;
			if (b->MemoryOwnership == IGD_AsyncSocket_MemoryOwnership_CHAIN) free(b->buffer);
			free(b);
		}
		IGD_Queue_Destroy(wr->streamedState->BufferQueue);
	...
	for(i=0; i < wr->NumberOfBuffers; ++i)
	{
		...
		if (wr->UserFree[i] == IGD_AsyncSocket_MemoryOwnership_CHAIN) { free(wr->Buffer[i]); }
	}
	...
	{
		IGD_LifeTime_Remove(wr->requestToken->timer, wr->requestToken->wcdo);
...
int IGD_CreateTokenStr(struct sockaddr* addr, int i, char* token)
{
...
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)token;
	struct IGD_WebRequest *wr;
	int zero = 0;
	...
	if (wcdo->SOCK != NULL && IGD_AsyncSocket_IsFree(wcdo->SOCK) == 0)
	{
		//
		// This connection needs to be disconnected first
		//
		wcdo->Closing = -1;
		IGD_AsyncSocket_Disconnect(wcdo->SOCK);
	}
	...
	if (wcdo->header != NULL)
	{
		...
		//
		IGD_DestructPacket(wcdo->header);	 // TODO: *********** CRASH ON EXIT SOMETIMES OCCURS HERE
		wcdo->header = NULL;
	}
//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
	if (wcdo->chunk != NULL)
...
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
	...
	//
	wr = IGD_Queue_DeQueue(wcdo->RequestQueue);
	while (wr!=NULL)
	{
		...
		IGD_WebClient_DestroyWebRequest(wr);
		wr = IGD_Queue_DeQueue(wcdo->RequestQueue);
	}

	IGD_Queue_Destroy(wcdo->RequestQueue);

	free(wcdo);
}

//
// Internal method to free resources associated with an IGD_WebClient object
//
// <param name="object">The IGD_WebClient to free</param>
void IGD_DestroyWebClient(void *object)
{
	struct IGD_WebClientManager *manager = (struct IGD_WebClientManager*)object;
	void *en;
	void *wcdo;
	char *key;
	int keyLength;
	...
	//
	en = IGD_HashTree_GetEnumerator(manager->DataTable);
	while (IGD_HashTree_MoveNext(en)==0)
	{
		//
		// Free the WebClientDataObject
		//
		IGD_HashTree_GetValue(en, &key, &keyLength, &wcdo);
		IGD_WebClient_DestroyWebClientDataObject(wcdo); // TODO: *********** BAD CRASH ON EXIT SOMETIMES OCCURS HERE
	}
	...
	//
	IGD_Queue_Destroy(manager->backlogQueue);
	IGD_DestroyHashTree(manager->idleTable);
	IGD_DestroyHashTree(manager->DataTable);
...
void IGD_WebClient_TimerInterruptSink(void *object)
{
	UNREFERENCED_PARAMETER( object );
}

void IGD_WebClient_ResetWCDO(struct IGD_WebClientDataObject *wcdo)
{
	IGD_WebClient_RequestToken rt = NULL;
	if (wcdo == NULL) return;
	rt = IGD_WebClient_GetRequestToken_FromStateObject(wcdo);	
	if (rt != NULL)
	{
		struct IGD_WebClient_PipelineRequestToken * plrt = ( struct IGD_WebClient_PipelineRequestToken * ) rt;
		
		// Check the cancel request in the timer list
		if ( plrt->timer != NULL ) IGD_LifeTime_Remove(plrt->timer, plrt);
	}
	wcdo->PAUSE = 0;
...
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
	...
	if (wcdo->header != NULL)
	{
		IGD_DestructPacket(wcdo->header);
		wcdo->header = NULL;
	}
}
//
...
void IGD_WebClient_TimerSink(void *object)
{
	void *enumerator;
	char IPAddress[24]; // Unoptimized version uses 300
	int IPAddressLength;
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)object;
	struct IGD_WebClientDataObject *wcdo2;
	...
	SEM_TRACK(WebClient_TrackLock("IGD_WebClient_TimerSink", 1, wcdo->Parent);)
	sem_wait(&(wcdo->Parent->QLock));
	if (IGD_Queue_IsEmpty(wcdo->RequestQueue)!=0)
	{
		//
		...
		if (wcdo->Parent->idleCount > MAX_IDLE_SESSIONS)
		{
			...
			enumerator = IGD_HashTree_GetEnumerator(wcdo->Parent->idleTable);
			IGD_HashTree_MoveNext(enumerator);
			IGD_HashTree_GetValue(enumerator, &key, &keyLength, &data);
			IGD_HashTree_DestroyEnumerator(enumerator);
			wcdo2 = IGD_GetEntry(wcdo->Parent->DataTable, key, keyLength);
			IGD_DeleteEntry(wcdo->Parent->DataTable, key, keyLength);
			IGD_DeleteEntry(wcdo->Parent->idleTable, key, keyLength);
			SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_TimerSink",2,wcdo->Parent);)
			sem_post(&(wcdo->Parent->QLock));
			IGD_WebClient_DestroyWebClientDataObject(wcdo2);
			return;
		}
		else
		{
			...
			//
			IPAddressLength = IGD_CreateTokenStr((struct sockaddr*)&(wcdo->remote), wcdo->IndexNumber, IPAddress);

			IGD_AddEntry(wcdo->Parent->idleTable, IPAddress, IPAddressLength, wcdo);
			++wcdo->Parent->idleCount;
			wcdo->SOCK = NULL;
			wcdo->DisconnectSent=0;
		}
	}
	SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_TimerSink", 3, wcdo->Parent);)
	sem_post(&(wcdo->Parent->QLock));
	//
	// Let the user know, the socket has been disconnected
	//
	if (DisconnectSocket != NULL) IGD_AsyncSocket_Disconnect(DisconnectSocket);
}
//
// Internal method called by IGD_WebServer, when a response was completed
//
// <param name="_wcdo">The associated WebClientDataObject</param>
void IGD_WebClient_FinishedResponse_Server(IGD_WebClient_StateObject _wcdo)
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)_wcdo;
...
void IGD_WebClient_FinishedResponse(IGD_AsyncSocket_SocketModule socketModule, struct IGD_WebClientDataObject *wcdo)
{
	int i;
	struct IGD_WebRequest *wr;
...
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
	...
	if (wcdo->SOCK == NULL || IGD_AsyncSocket_IsFree(wcdo->SOCK))
	{
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_FinishedResponse", 1, wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));
		IGD_WebClient_DestroyWebRequest(IGD_Queue_DeQueue(wcdo->RequestQueue));
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_FinishedResponse", 2, wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));
		return;
	}
	...
	if (wr != NULL)
	{
		...
		//
		IGD_WebClient_DestroyWebRequest(wr);
		wr = IGD_Queue_PeekQueue(wcdo->RequestQueue);
		if (wr == NULL)
		{
			//
			...
			//
			if (IGD_IsChainBeingDestroyed(wcdo->Parent->Chain) == 0)
			{
				IGD_LifeTime_Add(wcdo->Parent->timer, wcdo, HTTP_SESSION_IDLE_TIMEOUT, &IGD_WebClient_TimerSink, &IGD_WebClient_TimerInterruptSink);		
			}
		}
	}
	SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_FinishedResponse", 2 ,wcdo->Parent);)
	...
	if (wr != NULL)
	{
		...
		if (wcdo->PipelineFlag != PIPELINE_NO)
		{
			...
			for(i = 0; i < wr->NumberOfBuffers; ++i)
			{
				...
				//
				IGD_AsyncSocket_Send(wcdo->SOCK, wr->Buffer[i], wr->BufferLength[i], IGD_AsyncSocket_MemoryOwnership_STATIC);
			}
		}
	}
	if (wcdo->PipelineFlag == PIPELINE_NO)
...
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)webstate;
	struct IGD_WebRequest *wr = NULL;
...
//
// <param name="socketModule">The IGD_WebClient token</param>
// <param name="wcdo">The associated WebClientDataObject</param>
...
void IGD_WebClient_ProcessChunk(IGD_AsyncSocket_SocketModule socketModule, struct IGD_WebClientDataObject *wcdo, char *buffer, int *p_beginPointer, int endPointer)
{
	char *hex, *tmp;
	int i;
	...
	if (wcdo->Parent!=NULL) 
	{
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_ProcessChunk",1,wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));
	}
	wr = IGD_Queue_PeekQueue(wcdo->RequestQueue);
	if (wcdo->Parent!=NULL) 
	{
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_ProcessChunk",2,wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));
	}
	...
	if (wcdo->chunk==NULL)
	{
		...
		//
		if ((wcdo->chunk = (struct IGD_WebClient_ChunkData*)malloc(sizeof(struct IGD_WebClient_ChunkData))) == NULL) ILIBCRITICALEXIT(254);
		memset(wcdo->chunk,0,sizeof(struct IGD_WebClient_ChunkData));
		...
		case STARTCHUNK:
			...
			for(i=2;i<endPointer;++i)
			{
				if (buffer[i - 2]=='\r' && buffer[i - 1]=='\n')
				{
					...
					//
					pr = IGD_ParseString(buffer,0,i-2,";",1);
				...
				for (i=2;i<=endPointer;++i)
				{
					if (buffer[i-2]=='\r' && buffer[i-1]=='\n')
					{
						...
						if (i==2)
						{
							...
							if (IGD_AsyncSocket_IsFree(socketModule)==0)
							{
								//
								...
								IGD_WebClient_FinishedResponse(wcdo->SOCK,wcdo);
							}
						}
						else
						{
...
//
// <param name="socketModule">The underlying IGD_AsyncSocket</param>
// <param name="buffer">The receive buffer</param>
...
void IGD_WebClient_OnData(IGD_AsyncSocket_SocketModule socketModule, char* buffer, int *p_beginPointer, int endPointer, void (**InterruptPtr)(void *socketModule, void *user), void **user, int *PAUSE)
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)(*user);
	struct IGD_WebRequest *wr;
	...
	if (wcdo->Server == 0)
	{
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnData", 1, wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));
	}
	wr = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(wcdo->RequestQueue);
	if (wcdo->Server == 0)
	{
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnData", 2, wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));
	}
	if (wr == NULL)
	{
			...
			while (i <= (endPointer - (*p_beginPointer)) - 4)
			{
				...
				if (i > MAX_HTTP_HEADER_SIZE)
				{
					...
					if (wcdo->header == NULL)
					{
						...
						//
						tph = IGD_ParsePacketHeader(buffer,*p_beginPointer,endPointer-(*p_beginPointer));
						if (tph == NULL) ILIBCRITICALEXIT(253); // TODO: Handle this better?
						
						//
						// We need to free this memory, so we need to copy this packet
						//
						wcdo->header = IGD_ClonePacket(tph);
						IGD_DestructPacket(tph);
					}
					...
					buffer[*p_beginPointer+i+3]=='\n')
				{
					...
					wcdo->source.sin_addr.s_addr = IGD_AsyncSocket_GetRemoteInterface(socketModule);
					wcdo->source.sin_port = htons(IGD_AsyncSocket_GetRemotePort(socketModule));
					*/
					IGD_AsyncSocket_GetRemoteInterface(socketModule, (struct sockaddr*)(&wcdo->source));
					...
					if (wcdo->header == NULL)
					{
						wcdo->header = IGD_ParsePacketHeader(buffer, *p_beginPointer, endPointer - (*p_beginPointer));
					}
					if (wcdo->header != NULL)
					{
						//
						...
						if (wcdo->Server!=0 && wcdo->header->DirectiveObjLength>7 && memcmp(wcdo->header->DirectiveObj,"http://",7)==0)
						{
							...
							IGD_ParseUri(wcdo->header->DirectiveObj,&tempIP,&tempPort,&tempPath);
							if (strcmp(tempIP,IGD_IPAddress_ToDottedQuad(IGD_WebServer_GetLocalInterface(wr->user2)))==0 && tempPort==IGD_WebServer_GetPortNumber(wr->user1))
							{
								//
...
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
						...
						tmp = IGD_Queue_PeekQueue(wcdo->RequestQueue);
						if (wcdo->RequestQueue != NULL && tmp != NULL)
						{
							if (((struct IGD_WebRequest*)tmp)->IsHeadRequest != 0)
							{
								wcdo->BytesLeft = 0;
								wcdo->WaitForClose = 0;
								wcdo->Chunked = 0;
						...
						if (wcdo->BytesLeft == 0)
						{
							...
							*p_beginPointer = *p_beginPointer + i + 4;
							...
							IGD_WebClient_FinishedResponse(socketModule,wcdo);
						}
						else
						{
							//
								...
								else
								{
									...
									if (IGD_AsyncSocket_IsFree(socketModule)==0)
									{
										...
										*p_beginPointer = i+4+zero;
										...
										// for the body
										//
										tph = IGD_ClonePacket(wcdo->header);
										IGD_DestructPacket(wcdo->header);
										wcdo->header = tph;
									}
								}
...
//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
							else
							{
								...
								//
								IGD_WebClient_ProcessChunk(socketModule,wcdo,buffer+i+4,&zero,(endPointer - (*p_beginPointer) - (i+4)));
								*p_beginPointer = i+4+zero;
								tph = IGD_ClonePacket(wcdo->header);
								IGD_DestructPacket(wcdo->header);
								wcdo->header = tph;
...
//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
		else
		{
			...
			//
			IGD_WebClient_ProcessChunk(socketModule,wcdo,buffer,p_beginPointer,endPointer);
		}
//{{{ <--REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT }}}
	}
	if (IGD_AsyncSocket_IsFree(socketModule)==0)
...
void IGD_WebClient_OnSendOKSink(IGD_AsyncSocket_SocketModule socketModule, void *user)
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)user;
	struct IGD_WebRequest *wr = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(wcdo->RequestQueue);
...
//
// Internal method dispatched by the OnConnect event of the underlying IGD_AsyncSocket
//
// <param name="socketModule">The underlying IGD_AsyncSocket</param>
...
void IGD_WebClient_OnConnect(IGD_AsyncSocket_SocketModule socketModule, int Connected, void *user)
{
	int i;
	struct IGD_WebRequest *r;
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)user;
	#ifndef MICROSTACK_NOTLS
	int adsize;
	...
	//printf("IGD_WebClient_OnConnect(). Connected=%d, DisconnectSent=%d\r\n", Connected, wcdo->DisconnectSent);
	...
	if (Connected != 0 && wcdo->DisconnectSent == 0)
	{
		...
		{
			certs = IGD_AsyncSocket_SslGetCerts(socketModule);
			adsize = IGD_AsyncSocket_GetRemoteInterface(socketModule, (struct sockaddr*)&ad);
			...
			if (wcdo->Parent->OnSslConnection(wcdo, certs, &ad, wcdo->Parent->user) == 0)
			{
				...
				//IGD_AsyncSocket_Disconnect(socketModule);
				//return;
			}
		}
		#endif
		...
		IGD_AsyncSocket_GetLocalInterface(socketModule, (struct sockaddr*)&(wcdo->LocalIP));
		
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnConnect", 1, wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));
		r = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(wcdo->RequestQueue);
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnConnect", 2, wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));
		if (r != NULL)
		{
			for(i = 0; i < r->NumberOfBuffers; ++i)
			{
				IGD_AsyncSocket_Send(socketModule, r->Buffer[i], r->BufferLength[i], (enum IGD_AsyncSocket_MemoryOwnership)-1);
			}
			if (r->streamedState != NULL) IGD_WebClient_OnSendOKSink(socketModule, wcdo);
		}
	}
	else
	{
		...
		//
	    SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnConnect", 3, wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));

		keyLength = IGD_CreateTokenStr((struct sockaddr*)&(wcdo->remote), wcdo->IndexNumber, key);

		IGD_DeleteEntry(wcdo->Parent->DataTable, key, keyLength);
		IGD_DeleteEntry(wcdo->Parent->idleTable, key, keyLength);
		
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnConnect", 4, wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));

		IGD_WebClient_DestroyWebClientDataObject(wcdo);
	}
}
//
...
// <param name="socketModule">The underlying IGD_AsyncSocket</param>
// <param name="user">The associated WebClientDataObject</param>
void IGD_WebClient_OnDisconnectSink(IGD_AsyncSocket_SocketModule socketModule, void *user)
{
	char *buffer;
	...
	//printf("IGD_WebClient_OnDisconnectSink()\r\n");
	...
	else if (IGD_Queue_PeekQueue(wcdo->RequestQueue) != NULL && wcdo->WaitForClose != 0 && wcdo->CancelRequest == 0)
	{
	...
	if (wcdo->Closing == 0) 
	{
		if (IGD_Queue_PeekQueue(wcdo->RequestQueue) != NULL && wcdo->CancelRequest == 0)
		{
			//
	...
	if (wcdo->WaitForClose != 0 && wcdo->CancelRequest == 0)
	{
		...
		//
		IGD_AsyncSocket_GetBuffer(socketModule,&buffer,&BeginPointer,&EndPointer);
		
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnDisconnect", 1, wcdo->Parent);)
		sem_wait(&(wcdo->Parent->QLock));
		wr = (struct IGD_WebRequest*)IGD_Queue_DeQueue(wcdo->RequestQueue);
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnDisconnect", 2, wcdo->Parent);)
		sem_post(&(wcdo->Parent->QLock));
		wcdo->InitialRequestAnswered = 1;
		//{{{ REMOVE_THIS_FOR_HTTP/1.0_ONLY_SUPPORT--> }}}
		//wcdo->PipelineFlag = PIPELINE_NO;
		...
		if (wr != NULL) IGD_WebClient_DestroyWebRequest(wr);
		if (h != NULL) IGD_DestructPacket(h);
	}
	
	if (wcdo->Closing != 0) { return; }

	SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnDisconnect", 3, wcdo->Parent);)
	sem_wait(&(wcdo->Parent->QLock));
	wr = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(wcdo->RequestQueue);
	SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnDisconnect", 4, wcdo->Parent);)
	sem_post(&(wcdo->Parent->QLock));
	...
	if (wr != NULL)
	{
		...
		if (wcdo->InitialRequestAnswered == 0 && wcdo->CancelRequest == 0)
		{
			...
			if (IGD_AsyncSocket_IsFree(socketModule) != 0)
			{
				...
				// the pending request
				//
				SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnDisconnect", 5, wcdo->Parent);)
				sem_wait(&(wcdo->Parent->QLock));
				wr = (struct IGD_WebRequest*)IGD_Queue_DeQueue(wcdo->RequestQueue);
				SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnDisconnect", 6, wcdo->Parent);)
				sem_post(&(wcdo->Parent->QLock));
				if (wr != NULL)
				{
					IGD_WebClient_DestroyWebRequest(wr);
			...
			SEM_TRACK(WebClient_TrackLock("IGD_WebClient_OnDisconnect", 7, wcdo->Parent);)
			sem_wait(&(wcdo->Parent->QLock));
			wr = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(wcdo->RequestQueue);
			SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_OnDisconnect", 8, wcdo->Parent);)
			sem_post(&(wcdo->Parent->QLock));
			if (wr == NULL) { return; }
		}
		...
		wcdo->Closing = 0;
		IGD_Queue_EnQueue(wcdo->Parent->backlogQueue, wcdo);
	}
	wcdo->CancelRequest = 0;
}
...
void IGD_WebClient_PreProcess(void* WebClientModule, fd_set *readset, fd_set *writeset, fd_set *errorset, int* blocktime)
{
	struct IGD_WebClientManager *wcm = (struct IGD_WebClientManager*)WebClientModule;
	struct IGD_WebClientDataObject *wcdo;
	int OK = 0;
	int i;
	...
	//
	SEM_TRACK(WebClient_TrackLock("IGD_WebClient_PreProcess",1,wcm);)
	sem_wait(&(wcm->QLock));
	while (OK == 0 && IGD_Queue_IsEmpty(wcm->backlogQueue) == 0)
	{
		OK = 1;
		for(i=0;i<wcm->socksLength;++i)
		{
			if (IGD_AsyncSocket_IsFree(wcm->socks[i]) != 0)
			{
				OK = 0;
				wcdo = (struct IGD_WebClientDataObject *)IGD_Queue_DeQueue(wcm->backlogQueue);
				if (wcdo != NULL)
				{
					wcdo->Closing = 0;
					...
					#ifndef MICROSTACK_NOTLS
					if (wcm->ssl_ctx != NULL) IGD_AsyncSocket_SetSSLContext(wcdo->SOCK, wcm->ssl_ctx, 0);
					#endif
				}
			}
			if (IGD_Queue_IsEmpty(wcm->backlogQueue) != 0) break;
		}
	}
	SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_PreProcess", 2, wcm);)
	sem_post(&(wcm->QLock));
}

//
// Internal method dispatched by either IGD_WebServer or IGD_WebClient, to recheck the headers
//
...
//
// Internal method called by the IGD_WebServer module to create a WebClientDataObject
//
...
IGD_WebClient_StateObject IGD_CreateWebClientEx(IGD_WebClient_OnResponse OnResponse, IGD_AsyncSocket_SocketModule socketModule, void *user1, void *user2)
{
	...
	if ((wcdo = (struct IGD_WebClientDataObject*)malloc(sizeof(struct IGD_WebClientDataObject))) == NULL) ILIBCRITICALEXIT(254);
	memset(wcdo, 0 , sizeof(struct IGD_WebClientDataObject));
	wcdo->Parent = NULL;
	wcdo->RequestQueue = IGD_Queue_Create();
	wcdo->Server = 1;
	wcdo->SOCK = socketModule;
	wcdo->PendingConnectionIndex = -1;

	if ((wr = (struct IGD_WebRequest*)malloc(sizeof(struct IGD_WebRequest))) == NULL) ILIBCRITICALEXIT(254);
	memset(wr, 0, sizeof(struct IGD_WebRequest));
	wr->OnResponse = OnResponse;
	wr->user1 = user1;
	wr->user2 = user2;
	IGD_Queue_EnQueue(wcdo->RequestQueue, wr);
	return wcdo;
}

/*! \fn IGD_CreateWebClient(int PoolSize,void *Chain)
	\brief Constructor to create a new IGD_WebClient
	\param PoolSize The max number of IGD_AsyncSockets to have in the pool
	\param Chain The chain to add this module to. (Chain must <B>not</B> be running)
	\returns An IGD_WebClient
*/
IGD_WebClient_RequestManager IGD_CreateWebClient(int PoolSize, void *Chain)
{
	int i;
	struct IGD_WebClientManager *RetVal;
	
	if (Chain == NULL) return NULL;

	if ((RetVal = (struct IGD_WebClientManager*)malloc(sizeof(struct IGD_WebClientManager))) == NULL) ILIBCRITICALEXIT(254);
	memset(RetVal, 0, sizeof(struct IGD_WebClientManager));
	RetVal->MaxConnectionsToSameServer = 1;
	
	RetVal->Destroy = &IGD_DestroyWebClient;
	RetVal->PreSelect = &IGD_WebClient_PreProcess;
	//RetVal->PostSelect = &IGD_WebClient_PreProcess;
	...
	RetVal->backlogQueue = IGD_Queue_Create();
	RetVal->DataTable = IGD_InitHashTree();
	RetVal->idleTable = IGD_InitHashTree();

	
	IGD_AddToChain(Chain,RetVal);
	RetVal->timer = IGD_GetBaseTimer(Chain); //IGD_CreateLifeTime(Chain);
	...
	{
		RetVal->socks[i] = IGD_CreateAsyncSocketModule(
			Chain,
			INITIAL_BUFFER_SIZE,
			&IGD_WebClient_OnData,
			&IGD_WebClient_OnConnect,
			&IGD_WebClient_OnDisconnectSink,
			&IGD_WebClient_OnSendOKSink);
		//
		// We want to know about any buffer reallocations, because we may need to fix some things
		//
		IGD_AsyncSocket_SetReAllocateNotificationCallback(RetVal->socks[i], &IGD_WebClient_OnBufferReAllocate);
	}
	return((void*)RetVal);
}
...
{
	struct IGD_WebClientManager *wcm = (struct IGD_WebClientManager*)WebClient;
	wcm->MaxConnectionsToSameServer = maxConnections;
}

/*! \fn IGD_WebClient_PipelineRequest(IGD_WebClient_RequestManager WebClient, struct sockaddr_in *RemoteEndpoint, struct packetheader *packet, IGD_WebClient_OnResponse OnResponse, void *user1, void *user2)
	\brief Queues a new web request
	\param WebClient The IGD_WebClient to queue the requests to
								...
								void *user2)
{
	...
	bufferLength = IGD_GetRawPacket(packet,&buffer);
	IGD_DestructPacket(packet);
	return(IGD_WebClient_PipelineRequestEx(WebClient, RemoteEndpoint, buffer, bufferLength, IGD_AsyncSocket_MemoryOwnership_CHAIN, NULL, 0, 0, OnResponse, user1, user2));
}

IGD_WebClient_RequestToken IGD_WebClient_PipelineRequestEx2(
	IGD_WebClient_RequestManager WebClient, 
	struct sockaddr *RemoteEndpoint, 
	char *headerBuffer,
	...
	int bodyBuffer_FREE,
	IGD_WebClient_OnResponse OnResponse,
	struct IGD_WebClient_StreamedRequestState *state,
	void *user1,
	void *user2)
{
	int ForceUnBlock = 0;
	...
	struct IGD_WebClientManager *wcm = (struct IGD_WebClientManager*)WebClient;
	struct IGD_WebClientDataObject *wcdo;
	struct IGD_WebRequest *request;
	int i = 0;

	int indexWithLeast;
	int numberOfItems;

	if ((request = (struct IGD_WebRequest*)malloc(sizeof(struct IGD_WebRequest))) == NULL) ILIBCRITICALEXIT(254);

	memset(request, 0, sizeof(struct IGD_WebRequest));
	...
	if ((request->requestToken = (struct IGD_WebClient_PipelineRequestToken*)malloc(sizeof(struct IGD_WebClient_PipelineRequestToken))) == NULL) ILIBCRITICALEXIT(254);
	memset(request->requestToken, 0, sizeof(struct IGD_WebClient_PipelineRequestToken));
	...
	if (state != NULL)
	{
		// We were called from IGD_WebClient_PipelineStreamedRequest
	...
	// If there is IGD_AsyncSocket_MemoryOwnership_USER data in the buffers, change it to IGD_AsyncSocket_MemoryOwnership_CHAIN
	for(i = 0; i < request->NumberOfBuffers; ++i)
	{
		char* buf;
		if (request->UserFree[i] == IGD_AsyncSocket_MemoryOwnership_USER)
		{
			...
			request->UserFree[i] = IGD_AsyncSocket_MemoryOwnership_CHAIN;
		}
	}
	...
	//
	SEM_TRACK(WebClient_TrackLock("IGD_WebClient_PipelineRequestEx", 1, wcm);)
	sem_wait(&(wcm->QLock));
	...
	if (wcm->MaxConnectionsToSameServer > 1)
	{
		...
		{
			RequestTokenLength = IGD_CreateTokenStr(RemoteEndpoint, i, RequestToken);
			if (IGD_HasEntry(wcm->DataTable, RequestToken, RequestTokenLength) == 0)
			{
				// A free slot!
				break;
			}
		...
		if (i == wcm->MaxConnectionsToSameServer)
		{
			...
			{
				RequestTokenLength = IGD_CreateTokenStr(RemoteEndpoint, i, RequestToken);
				wcdo = (struct IGD_WebClientDataObject*)IGD_GetEntry(wcm->DataTable, RequestToken, RequestTokenLength);
				if (wcdo == NULL) ILIBCRITICALEXIT(253); // TODO: Better handling
				
				if (numberOfItems == -1)
				{
					numberOfItems = IGD_LinkedList_GetCount(wcdo->RequestQueue);
					indexWithLeast = i;
				}
				else
				{
					if (IGD_LinkedList_GetCount(wcdo->RequestQueue) < numberOfItems)
					{
						numberOfItems = IGD_LinkedList_GetCount(wcdo->RequestQueue);
						indexWithLeast = i;
					}
			...
			RequestTokenLength = IGD_CreateTokenStr(RemoteEndpoint, indexWithLeast, RequestToken);
		}
	}
	else
	{
		i = 0;
		RequestTokenLength = IGD_CreateTokenStr(RemoteEndpoint, 0, RequestToken);
	}
	...
	if (IGD_HasEntry(wcm->DataTable, RequestToken, RequestTokenLength)!=0)
	{
		...
		if (IGD_Queue_IsEmpty(wcdo->RequestQueue) != 0)
		{
				...
				IGD_Queue_EnQueue(wcdo->RequestQueue, request);

				// Take out of Idle State
				wcm->idleCount = wcm->idleCount == 0?0:wcm->idleCount-1;
				IGD_DeleteEntry(wcm->idleTable, RequestToken, RequestTokenLength);
				IGD_LifeTime_Remove(wcm->timer, wcdo);
				if (wcdo->DisconnectSent == 0 && (wcdo->SOCK == NULL || IGD_AsyncSocket_IsFree(wcdo->SOCK)))
				{
					// If this was in our idleTable, then most likely the select doesn't know about
					// it, so we need to force it to unblock
					IGD_Queue_EnQueue(wcm->backlogQueue, wcdo);	
					ForceUnBlock = 1;
				}
				else if (wcdo->SOCK != NULL)
				{
					...
					if (wcdo->WaitForClose == 0)
					{
						for(i = 0; i < request->NumberOfBuffers; ++i)
						{
							...
							IGD_AsyncSocket_Send(wcdo->SOCK, request->Buffer[i], request->BufferLength[i], IGD_AsyncSocket_MemoryOwnership_STATIC);
						}
						if (request->streamedState != NULL)
						{
							IGD_WebClient_OnSendOKSink(wcdo->SOCK, wcdo);
						}
					}
				}
			}
	...
	else
	{
		...
		IGD_Queue_EnQueue(wcdo->RequestQueue, request);
		IGD_AddEntry(wcm->DataTable, RequestToken, RequestTokenLength, wcdo);
		if (wcdo->DisconnectSent == 0)
		{
	...
	SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_PipelineRequestEx", 2, wcm);)
	sem_post(&(wcm->QLock));
	if (ForceUnBlock != 0) IGD_ForceUnBlockChain(wcm->Chain);
	SESSION_TRACK(request->requestToken, NULL, "PipelinedRequestEx");
	return(request->requestToken);
}

/*! \fn IGD_WebClient_PipelineRequestEx(
	IGD_WebClient_RequestManager WebClient, 
	...
	\brief Queues a new web request
	\par
	This method differs from IGD_WebClient_PiplineRequest, in that this method
...
IGD_WebClient_RequestToken IGD_WebClient_PipelineRequestEx(
	...
	void *user2)
{
	return(IGD_WebClient_PipelineRequestEx2(WebClient, RemoteEndpoint, headerBuffer, headerBufferLength, headerBuffer_FREE, bodyBuffer, bodyBufferLength, bodyBuffer_FREE, OnResponse, NULL, user1, user2));
}
IGD_WebClient_RequestToken IGD_WebClient_PipelineRequest2(
								IGD_WebClient_RequestManager WebClient, 
								struct sockaddr *RemoteEndpoint, 
								struct packetheader *packet,
								IGD_WebClient_OnResponse OnResponse,
								...
								void *user2)
{
	...
	bufferLength = IGD_GetRawPacket(packet, &buffer);
	IGD_DestructPacket(packet);
	return(IGD_WebClient_PipelineRequestEx2(WebClient, RemoteEndpoint, buffer, bufferLength, IGD_AsyncSocket_MemoryOwnership_CHAIN, NULL, 0, 0, OnResponse, state, user1, user2));
}
...
{
	return(token == NULL?NULL:((struct IGD_WebClientDataObject*)token)->header);
}

/*! \fn IGD_WebClient_DeleteRequests(IGD_WebClient_RequestManager WebClientToken,char *IP,int Port)
	\brief Deletes all pending requests to a specific IP/Port combination
	\param WebClientToken The IGD_WebClient to purge
...
void IGD_WebClient_DeleteRequests(IGD_WebClient_RequestManager WebClientToken, struct sockaddr* addr)
{
	...
	struct IGD_WebRequest *wr;
	struct IGD_WebClientDataObject *wcdo = NULL;
	struct IGD_WebClientManager *wcm = (struct IGD_WebClientManager*)WebClientToken;
	
	if (wcm == NULL) {return;}
	RemoveQ = IGD_Queue_Create();

	for(i=0;i<wcm->MaxConnectionsToSameServer;++i)
	{
		RequestTokenLength = IGD_CreateTokenStr(addr, i, RequestToken);

		//
		// Are there any pending requests to this IP/Port combo?
		//
		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_DeleteRequests", 1, wcm);)
		sem_wait(&(wcm->QLock));
		if (IGD_HasEntry(wcm->DataTable, RequestToken, RequestTokenLength) != 0)
		{
			//
			...
			if (wcdo!=NULL)
			{
				...
				//
				IGD_LinkedList_Remove_ByData(wcm->backlogQueue, wcdo);
				while (IGD_Queue_IsEmpty(wcdo->RequestQueue) == 0)
				{
					//
					// Put all the pending requests into this queue, so we can trigger them outside of this lock
					//
					wr = (struct IGD_WebRequest*)IGD_Queue_DeQueue(wcdo->RequestQueue);
					IGD_Queue_EnQueue(RemoveQ, wr);
				}
			
				IGD_LifeTime_Remove(wcdo->Parent->timer, wcdo);
			}
		}
		SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_DeleteRequests",2,wcm);)
		sem_post(&(wcm->QLock));
	}
	...
	//
	while (IGD_Queue_IsEmpty(RemoveQ) == 0)
	{
		//
		// Tell the user, we are aborting these requests
		//
		wr = (struct IGD_WebRequest*)IGD_Queue_DeQueue(RemoveQ);
		if (wr != NULL && wr->OnResponse != NULL)
		{
			wr->OnResponse(				
				NULL,
	...
	if (wcdo != NULL && wcdo->Parent != NULL)
	{
		...
		{
			IGD_AsyncSocket_Disconnect(wcm->socks[wcdo->PendingConnectionIndex]);
			wcdo->PendingConnectionIndex = -1;
		}
	}
	...
	IGD_Queue_Destroy(RemoveQ);
}

/*! \fn IGD_WebClient_Resume(IGD_WebClient_StateObject wcdo)
...
{
	struct IGD_WebClientDataObject *d = (struct IGD_WebClientDataObject*)wcdo;
	
	if (d!=NULL)
	{
		d->PAUSE = 0;
		IGD_AsyncSocket_Resume(d->SOCK);
	}
}
...
/*! \fn IGD_WebClient_Disconnect(IGD_WebClient_StateObject wcdo)
	\brief Disconnects the underlying socket, of a client object.
	\par
	<b>Note</b>: This is <b>not</b> to be used to close an HTTP Session, as IGD_WebClient does not
	...
	be accomplished by calling \a IGD_WebClient_CancelRequest, with the RequestToken obtained from \a IGD_WebClient_PipelineRequest.
	\param wcdo WebClient State Object, obtained from \a IGD_CreateWebClientEx.
*/
void IGD_WebClient_Disconnect(IGD_WebClient_StateObject wcdo)
{
	...
	if (d!=NULL)
	{
		IGD_AsyncSocket_Disconnect(d->SOCK);
	}
}

void IGD_WebClient_CancelRequestEx2(IGD_WebClient_StateObject wcdo, void *userRequest)
{
	void *node,*nextnode;
	struct IGD_WebRequest *wr;
	struct IGD_WebClientDataObject *_wcdo = (struct IGD_WebClientDataObject*)wcdo;
	int HeadDeleted = 0;
	void *head;
	int BeginPointer=0;
	int EndPointer=0;
	...
	if (wcdo != NULL)
	{
		PendingRequestQ = IGD_Queue_Create();

		SEM_TRACK(WebClient_TrackLock("IGD_WebClient_CancelRequest",1,_wcdo->Parent);)
		sem_wait(&(_wcdo->Parent->QLock));

		head = node = IGD_LinkedList_GetNode_Head(_wcdo->RequestQueue);
		while (node != NULL)
		{
			nextnode = IGD_LinkedList_GetNextNode(node);

			wr = (struct IGD_WebRequest*)IGD_LinkedList_GetDataFromNode(node);
			if (wr->requestToken == userRequest)
			{
				...
				IGD_Queue_EnQueue(PendingRequestQ, wr);
		...
		IGD_LifeTime_Remove(_wcdo->Parent->timer, _wcdo);
		if (HeadDeleted != 0 && _wcdo->SOCK != NULL)
		{
			...
			SEM_TRACK(WebClient_TrackUnLock("IGD_WebClient_CancelRequest", 2, _wcdo->Parent);)
			sem_post(&(_wcdo->Parent->QLock));

			IGD_WebClient_Disconnect(_wcdo);

			SEM_TRACK(WebClient_TrackLock("IGD_WebClient_CancelRequest", 3, _wcdo->Parent);)
			sem_wait(&(_wcdo->Parent->QLock));
			...
			// to be handled
			//
			if (IGD_Queue_PeekQueue(_wcdo->RequestQueue) != NULL)
			{
				//
				// Queue the session to be reconnected
				//
				IGD_Queue_EnQueue(_wcdo->Parent->backlogQueue, _wcdo);				
			}
		}
		...
		wr = (struct IGD_WebRequest*)IGD_Queue_DeQueue(PendingRequestQ);
		while (wr != NULL)
		{			
			if (wr->OnResponse != NULL)
			{
		...
		IGD_Queue_Destroy(PendingRequestQ);
	}
}

void IGD_WebClient_CancelRequestEx(void *RequestToken)
{
	IGD_WebClient_CancelRequestEx2(((struct IGD_WebClient_PipelineRequestToken*)RequestToken)->wcdo,RequestToken);
}

/*! \fn IGD_WebClient_CancelRequest(IGD_WebClient_RequestToken RequestToken)
	\brief Cancels a pending request via the RequestToken received when making the request.
	\param RequestToken The identifier obtained from calls to \a IGD_WebClient_PipelineRequest.
*/
void IGD_WebClient_CancelRequest(IGD_WebClient_RequestToken RequestToken)
{
	if (RequestToken != NULL)
	{
		IGD_LifeTime_Add(((struct IGD_WebClient_PipelineRequestToken*)RequestToken)->timer, RequestToken, 0, IGD_WebClient_CancelRequestEx, NULL);
	}
}
/*! \fn IGD_WebClient_GetRequestToken_FromStateObject(IGD_WebClient_StateObject WebStateObject)
	\brief Obtains the Request Token associated with the specified WebReader (response) token.
	\param WebStateObject The response token obtained from the response handler passed into \a IGD_WebClient_PipelineRequest.
...
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)WebStateObject;
	struct IGD_WebRequest *wr;
...
/*! \fn IGD_WebClient_StateObject IGD_WebClient_GetStateObjectFromRequestToken(IGD_WebClient_RequestToken token)
	\brief Obtain the user object that was associated with a request, from the request token
	\param token The token that was obtained from a call to \a IGD_WebClient_PipelineRequest or \a IGD_WebClient_PipelineRequestEx
*/
IGD_WebClient_StateObject IGD_WebClient_GetStateObjectFromRequestToken(IGD_WebClient_RequestToken token)
{
	return(((struct IGD_WebClient_PipelineRequestToken*)token)->wcdo);
}
...
/*! \fn IGD_WebClient_RequestToken IGD_WebClient_PipelineStreamedRequest(IGD_WebClient_RequestManager WebClient,struct sockaddr_in *RemoteEndpoint,struct packetheader *packet,IGD_WebClient_OnResponse OnResponse,IGD_WebClient_OnSendOK OnSendOK,void *user1,void *user2)
	...
	\param OnSendOK Function pointer that will trigger when a connection has been established and again when all calls to IGD_WebClient_StreamRequestBody() have completed
	\param user1 User state object
	\param user2 User state object
*/
IGD_WebClient_RequestToken IGD_WebClient_PipelineStreamedRequest(IGD_WebClient_RequestManager WebClient,struct sockaddr *RemoteEndpoint, struct packetheader *packet,IGD_WebClient_OnResponse OnResponse,IGD_WebClient_OnSendOK OnSendOK,void *user1,void *user2)
{
	struct IGD_WebClient_StreamedRequestState *state;

	if ((state = (struct IGD_WebClient_StreamedRequestState*)malloc(sizeof(struct IGD_WebClient_StreamedRequestState))) == NULL) ILIBCRITICALEXIT(254);
	memset(state,0,sizeof(struct IGD_WebClient_StreamedRequestState));

	state->BufferQueue = IGD_Queue_Create();
	state->OnSendOK = OnSendOK;
	state->doNotSendRightAway = 1;

	IGD_AddHeaderLine(packet,"Transfer-Encoding",17,"chunked",7);
	
	return(IGD_WebClient_PipelineRequest2(WebClient,RemoteEndpoint,packet,OnResponse,state,user1,user2));
}

/*! \fn void IGD_WebClient_StreamRequestBody(IGD_WebClient_RequestToken token, char *body,int bodyLength, enum IGD_AsyncSocket_MemoryOwnership MemoryOwnership,int done)
	\brief Streams part of the request body
	\param token The RequestToken received from a call to IGD_WebClient_PipelineStreamedRequest()
...
{
	struct IGD_WebClient_PipelineRequestToken *t = (struct IGD_WebClient_PipelineRequestToken*)token;
	struct IGD_WebRequest *wr;
	...
	{
		wr = (struct IGD_WebRequest*)IGD_Queue_PeekQueue(t->wcdo->RequestQueue);
		if (body != NULL && bodyLength > 0)
		{
			hexLen = snprintf(hex, 16, "%X\r\n", bodyLength);
			result = IGD_AsyncSocket_Send(t->wcdo->SOCK, hex, hexLen, IGD_AsyncSocket_MemoryOwnership_USER);
			if (result >= 0)
			{
				result = IGD_AsyncSocket_Send(t->wcdo->SOCK, body ,bodyLength, MemoryOwnership);
				if (result >= 0)
				{
					result = IGD_AsyncSocket_Send(t->wcdo->SOCK, "\r\n", 2, IGD_AsyncSocket_MemoryOwnership_STATIC);
				}
			}
			else if (MemoryOwnership == IGD_AsyncSocket_MemoryOwnership_CHAIN)
			{
				free(body);
			}
		}
		if (result >= 0 && done != 0)
		{
			result = IGD_AsyncSocket_Send(t->wcdo->SOCK, "0\r\n\r\n", 5, IGD_AsyncSocket_MemoryOwnership_STATIC);
		}
...
/*! \fn void IGD_WebClient_Parse_ContentRange(char *contentRange, int *Start, int *End, int *TotalLength)
	\brief Parses an HTTP/206 Partial Content, Content-Range header, to obtain the range that was returned.
	\param contentRange The Content-Range header to parse. This can be obtained with a call to \a IGD_GetHeaderLine
...
void IGD_WebClient_Parse_ContentRange(char *contentRange, int *Start, int *End, int *TotalLength)
{
	...
	*TotalLength = 0;
	...
	pr = IGD_ParseString(contentRange,0,(int)strlen(contentRange)," ",1);
	//
	// bytes x-y/z
	//
	if (pr->NumResults == 2)
		...
		if (pr->FirstResult->datalength == 5 && memcmp(pr->FirstResult->data,"bytes",5)==0)
		{
			...
			if (pr2->NumResults == 2)
			{
				...
				*TotalLength = atoi(pr2->LastResult->data);
				pr3 = IGD_ParseString(pr2->FirstResult->data, 0, pr2->FirstResult->datalength, "-", 1);
				if (pr3->NumResults==2)
				{
...
/*! \fn int IGD_WebClient_Parse_Range(char *Range, long *Start, long *Length, long TotalLength)
	\brief Parses the Range request header, to obtain the requested range
	\param Range The Range header to parse. This can be obtained with a call to \a IGD_GetHeaderLine
...
enum IGD_WebClient_Range_Result IGD_WebClient_Parse_Range(char *Range, long *Start, long *Length, long TotalLength)
{
	...
	long x=-1;
	long y=-1;
	enum IGD_WebClient_Range_Result RetVal = IGD_WebClient_Range_Result_OK;

	*Start = 0;
	*Length = 0;

	pr = IGD_ParseString(Range, 0, (int)strlen(Range), "=", 1);
	if (pr->NumResults == 2 && pr->FirstResult->datalength==5 && memcmp(pr->FirstResult->data, "bytes",5)==0)
	{
		...
		{
			RetVal = IGD_WebClient_Range_Result_BAD_REQUEST;
		}
		IGD_DestructParserResults(pr2);
	}
	else
	...
	if (x >= 0 && y >= 0 && x <= TotalLength)
	{
		*Start = x;
		*Length = y;
		RetVal = IGD_WebClient_Range_Result_OK;
	}
	else if (RetVal == IGD_WebClient_Range_Result_OK)
	{
		RetVal = IGD_WebClient_Range_Result_INVALID_RANGE;
	}
	return(RetVal);
}
void IGD_WebClient_SetUser(IGD_WebClient_RequestManager manager, void *user)
{
	((struct IGD_WebClientManager*)manager)->user = user;
}
void* IGD_WebClient_GetUser(IGD_WebClient_RequestManager manager)
{
	return(((struct IGD_WebClientManager*)manager)->user);
}
void* IGD_WebClient_GetChain(IGD_WebClient_RequestManager manager)
{
	return(((struct IGD_WebClientManager*)manager)->Chain);
}
...
{
	struct IGD_WebClientManager *wcm = (struct IGD_WebClientManager *)manager;
...
{
	struct IGD_WebClientDataObject *wcdo = (struct IGD_WebClientDataObject*)socketModule;
	return IGD_AsyncSocket_GetLocalInterface(wcdo->SOCK, localAddress);
}
...
{
	return IGD_AsyncSocket_SslGetCert(((struct IGD_WebClientDataObject*)socketModule)->SOCK);
}

STACK_OF(X509)*	IGD_WebClient_SslGetCerts(void* socketModule)
{
	return IGD_AsyncSocket_SslGetCerts(((struct IGD_WebClientDataObject*)socketModule)->SOCK);
}
...
int IGD_WebClient_HasAllocatedClient(IGD_WebClient_RequestManager WebClient, struct sockaddr *remoteAddress)
{
	...
	addr1len = IGD_GetAddrBlob(remoteAddress, &addr1);

	// Get address 2 & check it.
	for(i = 0; i < wcm->socksLength; ++i)
	{
		sock = *((int*)IGD_AsyncSocket_GetSocket((IGD_AsyncSocket_SocketModule)wcm->socks[i]));
		if (sock != ~0)
		{
			l = IGD_AsyncSocket_GetRemoteInterface((IGD_AsyncSocket_SocketModule)wcm->socks[i], (struct sockaddr*)&addr);
			addr2len = IGD_GetAddrBlob((struct sockaddr*)&addr, &addr2);
...
int IGD_WebClient_GetActiveClientCount(IGD_WebClient_RequestManager WebClient)
{
	...
	for(i = 0; i < wcm->socksLength; ++i)
	{
		if ((*((int*)IGD_AsyncSocket_GetSocket((IGD_AsyncSocket_SocketModule)wcm->socks[i]))) != ~0) count++;
	}
	return count;
}
