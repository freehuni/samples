...
int DI_NETWORK_GetRouterAdvertisementFlags(char* interfaceName, bool* outMFlag, bool* outOFlag)
{
	int sd, status;
	int seq = time(NULL);
	...
	for (;;)
	{
		...
		for (nlm = (struct nlmsghdr *)nbuf; NLMSG_OK(nlm, msg_len); nlm = (struct nlmsghdr *)NLMSG_NEXT(nlm, msg_len))
		{
			...
			if (strcmp(if_name, interfaceName) != 0)
			{
				continue;
			}
...
int main()
{
	...
	DI_NETWORK_GetRouterAdvertisementFlags("enp2s0", &m, &o);
	printf("M flag : %d O flag: %d\n", m, o);
